...
## 2026-09-07 BUILD-08 The published report is not byte reproducible, so make all never leaves a clean tree

**Symptom.** Found while running the release checklist of phase E5, which
expected a clean tree after regenerating every asset from the committed data.
Nothing about the document changed and `git status` reported one file modified:

```text
$ make assets && make reports
  report  assets/reports/main_report.pdf (524 KiB)
  report  assets/reports/debug_report.pdf (356 KiB)

$ git status --porcelain
 M assets/reports/main_report.pdf
```

The twelve figures under `assets/figures/` regenerated byte for byte, which is
what phase A8c established, and `assets/reports/debug_report.pdf` did not move
because latexmk had nothing to do for it. The main report did move, and it moved
in a way that is not a content change:
...
$ cmp -l tracked new | wc -l
388395

$ diff <(strings tracked | grep 'CreationDate\|ModDate\|/ID') <(strings new | ...)
< /CreationDate (D:20260907023135+02'00')
< /ModDate     (D:20260907023135+02'00')
< /ID [<44CC94C575A24F3B5B820B2E5156CC03> <44CC94C575A24F3B5B820B2E5156CC03>]
---
> /CreationDate (D:20260907025322+02'00')
> /ModDate     (D:20260907025322+02'00')
> /ID [<BE1AA35F4EFD235927F799B17855C764> <BE1AA35F4EFD235927F799B17855C764>]

$ pdftotext -layout tracked tracked.txt && pdftotext -layout new new.txt && cmp tracked.txt new.txt
(silent, exit 0)
```

Three lines of metadata differ and the extracted text of the two files is byte
identical. `compare_report_text.py` agrees: 1572 numeric tokens across 51 pages,
no divergence.

**Root cause.** pdfTeX writes the wall clock into `/CreationDate` and
`/ModDate`, and derives the file `/ID` from it, unless `SOURCE_DATE_EPOCH` is
set in the environment. Nothing in the `Makefile`, in `report/.latexmkrc` or in
the workflow sets it. The 388395 differing bytes are not 388395 differences: the
`/ID` is written into the trailer and the changed length of the metadata object
shifts every cross reference offset after it, so the whole compressed tail
compares unequal from one changed timestamp. The thirteen byte size difference
is the same cause.

The consequence is narrow but real. `publish_assets.py` copies the built PDF
over the tracked one on every `make reports`, so `make all` from a clean,
committed tree always ends with a modified file that carries no new information.
An owner following the release steps sees a dirty tree and has to decide whether
it means anything, and the honest answer requires extracting the text of both
files to find out. That is exactly the kind of check that gets skipped once.

**Options.**

1. **Export `SOURCE_DATE_EPOCH` in the report targets**, from the commit date of
   `HEAD` or from a fixed value, which is the documented way to make pdfTeX
   deterministic. It is three lines in the `Makefile`. The cost is that the two
   currently tracked PDFs were built without it, so the first build under the
   new setting produces different bytes for both and both have to be
   republished, in a commit whose diff is two binary files and whose content
   change is nothing.
2. **Compare text rather than bytes and accept the churn.** This is what the
   repository already does everywhere it matters: the continuous integration
   reports job compares `pdftotext` output and says in a comment why byte
   comparison is the wrong instrument for a PDF, and phase A8b and A8c both
   quoted `compare_report_text.py` rather than `cmp`. The gap is only that
   nothing states the rebuilt PDF will differ, so a reader meets it as a
   surprise.
3. **Stop tracking the PDFs.** Rejected without much thought. The README links
   to them, a release attaches them, and a reader who cannot build LaTeX is
   exactly who they are for.

**Fix.** Option 2 for release 1.1.0, deliberately, and option 1 recorded as the
change to make in the publish phase of release 1.2.0, where both PDFs are
rebuilt and republished anyway and the byte change costs nothing extra. Applying
option 1 in the release commit itself would put a two binary file diff with no
content change into the commit that tags the release, which is the opposite of
what a release commit should be readable as. The tracked PDF was restored with
`git checkout -- assets/reports/main_report.pdf` after the checklist confirmed
the text was identical, and the release commit carries no asset change.

What the release steps now say, so it is not a surprise: after `make reports`,
`assets/reports/main_report.pdf` will show as modified with no content change,
and `python3 scripts/compare_report_text.py report/main.pdf
assets/reports/main_report.pdf` is the check that settles whether that is true.
...
$ pdftotext -layout on both, then cmp
text cmp exit 0

$ git checkout -- assets/reports/main_report.pdf && git status --porcelain
 M .github/workflows/ci.yml
 M CHANGELOG.md
 M CITATION.cff
 M CMakeLists.txt
 M cmake/version.hpp.in
```
