...
The measured knee sits at \textbf{three to five workers}, not at eight. Beyond
it the fitted slope is slightly negative: adding workers does not merely stop
helping, it costs a little.

The bandwidth probe explains why, and does so independently. Sweeping the STREAM
triad across worker counts on an idle machine gives
...
\midrule
GiB/s & 42.4 & 62.3 & 61.2 & 60.0 & 56.8 & 56.7 & 54.7 & 37.7 \\
\bottomrule
\end{tabular}
\end{center}

The memory system is saturated by four workers, stays flat to about twelve, and
falls away thereafter, reaching its worst value by a wide margin once every
hyperthread is engaged. A Jacobi sweep, whose access pattern is less regular than
a triad, saturates at the same point or slightly sooner, which is the three to
...
measurement on an idle machine moved the peak to four, with eight only 1.8
percent behind. The curve is flat enough between four and twelve that the
location of its maximum is not stable between runs, so the coincidence does not
survive and the inference is withdrawn. It is recorded here rather than deleted
because a plausible pattern that fails to reproduce is worth exactly one
sentence of warning to the next person who notices it.
