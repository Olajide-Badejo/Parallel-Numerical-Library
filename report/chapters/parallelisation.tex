...
time. What it asserts is OpenMP 4.5, and 4.5 is the whole of what the backend
uses. The reference document cited is OpenMP 6.0 \cite{openmp2024}, and the code
is deliberately restricted to constructs the compiler actually implements.
...
\textbf{The jthread pool} uses only what C++20 provides: \texttt{std::jthread}
...
statement that the frontend inside \texttt{nvcc} does not implement. So the host
compiler and the CUDA host compiler cannot be the same binary whichever released
GCC the project picks.

The resolution is a C ABI boundary. The \texttt{.cu} files are compiled by
\texttt{nvcc} driving GCC 14; everything else is compiled by the project
compiler, which was GCC 16 for the numbers in this report and is GCC 15.2.0 from
release 1.1.0 onwards; and every CUDA entry point is declared \texttt{extern
"C"} taking plain pointers and scalars. No libstdc++ type crosses, so the two
compilers never have to agree on a C++ ABI, only on the platform C ABI, which
they do by definition. This also satisfies the rule that backend files never
leak their model's types.
