...
\section{The traffic model correction makes the conclusion stronger}
...
overwrite it. It is worth being clear about which way that correction cuts,
because a correction that raises this project's own reported traffic looks at
first like an admission and is not one.

Under the conservative count the host Jacobi row is the one place in the device
comparison where the two devices are not used comparably well, and the gap is
large. Under the read for ownership count the host is moving a third more
traffic than it declares, so its achieved bandwidth and its efficiency both rise
by a third, while the device figures do not move at all: the GPU writes its
output through a memory system that does not pay the same charge, and its
declared and counted figures coincide. Corrected, host Jacobi uses its memory
system comparably well to the GPU, the Jacobi row stops being an exception, and
the remaining advantage of the graphics card collapses to roughly the ratio of
the two measured triads.

That is the stronger conclusion, not the weaker one, because it is the same
conclusion the other three kernels already gave independently. Red black Gauss
Seidel, red black SOR and conjugate gradient each show two devices used to
within a percentage point or two of one another, so the whole of the observed
speedup on those kernels is the memory system. A Jacobi row that also lands
there turns three agreeing measurements into four, and removes the one row a
reader could have pointed at to say that the implementations, and not the
hardware, were what differed.
...
\ref{sec:device-results} carries.

Until that sentence names an outcome, every claim in this chapter that would
change under the other model is stated conditionally, both counts are carried in
every table, and no conclusion here rests on one of them in a way that would
