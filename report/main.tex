...
% Every measured figure this report's prose quotes, one command each, written by
% scripts/gen_report_assets.py from summary.csv and the session manifest. A
% sentence asks for a command rather than carrying a number, so a re measurement
% rewrites the prose along with the tables, and a figure that stops being
% derivable is a build failure here rather than a stale claim on the page. This
% file is generated, is not tracked, and `make report` writes it before latexmk
% runs.
...
\begin{abstract}
\noindent
This report describes a C++20 numerical library in which twelve iterative
...
\include{chapters/parallelisation}
